...
def train_epoch(epoch, network, qpredictor, train_loader, loss_function, optimiser, num_atoms, dataset, device, verbose=False):
    ...
    for batch in tqdm.tqdm(train_loader, desc=f"Training epoch #{epoch}...", leave=False):
        x, labelled_atoms, _ = [t.to(device) for t in batch]
        ...
        #predict charges
        q0 = qpredictor(x0, labelled_atoms0)
        q1 = qpredictor(x1, labelled_atoms1)
        ...
        network_loss = mse_loss + args.scale*q_mse_loss + scale*(bond_energy + angle_energy + torsion_energy + NB_energy)
...
def validation(epoch, network, qpredictor, test0, test1, dataset, num_atoms, labelled_atoms, dataloader, 
    ...
    interpolation_out.clip(min=-999, max=9999)
...
def main(args):
    ...
    cfg = args.__dict__
    #qpredictor
    saved_mean, saved_std = np.load('/home/ebam/kacher1/molearn/DATA/KV1/meanval_stdval.npy', allow_pickle=True)
    ...
    dataset.meanval = saved_mean
    dataset.stdval = saved_std
    ...
    qpredictor.load_state_dict(torch.load("/home/ebam/kacher1/molearn/DATA/KV1/checkpoints_extra/epoch_241_0.0039714.pth")) #strict=False)
    
    for param in qpredictor.parameters():
