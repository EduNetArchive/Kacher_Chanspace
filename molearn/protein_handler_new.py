...
import numpy as np
from prody import showSubfamilySpectralOverlaps
import torch
import time
...
def get_convolutions(dataset, pdb_atom_names,
    ...
    for i, (atom1, res, _) in enumerate(pdb_atom_names):
        if atom1 in ["H1", "H2", "H3"] and "H" in amber_atoms[res]:
            unique_fixes.append((res, atom1, "H"))
            pdb_atom_names[i, 0] = "H"
            continue

        best_match = None
        best_ratio = None
        
        for atom2 in amber_atoms[res]:
            if atom1[0] != atom2[0]:
                continue

            ratio = fuzz.ratio(atom1, atom2)
            if best_ratio is None or ratio > best_ratio:
                best_ratio = ratio
                best_match = atom2
        
        if atom1 != best_match:
            unique_fixes.append((res, atom1, best_match))

        pdb_atom_names[i, 0] = best_match

    unique_fixes = np.unique(unique_fixes, axis=0)
    print("Renaming atom names for Amber style (res: oldatom -> newatom)")
    for res, atom1, atom2 in unique_fixes:
        print(f"{res}: {atom1} -> {atom2}")

    atom_names = [
        [
            amber_atoms[res][atom], 
            res,
            resid
        ]
