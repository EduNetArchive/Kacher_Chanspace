...
def train_epoch(epoch, network, qpredictor, train_loader, loss_function, optimiser, num_atoms, dataset, device, verbose=False):
    ...
    for batch in tqdm(train_loader, desc=f"Training epoch #{epoch}...", leave=False):
        ...
        elif not torch.all(torch.isfinite(q_out_interpolated)): 
            raise ValueError(f"q_out_interpolated is NaN/inf!!!")

        # print(q_interpolated, q_out_interpolated)
        ...
        if args.no_phys:
            network_loss = mse_loss + args.q_scale*q_mse_loss 
        else: 
            network_loss = mse_loss + args.q_scale*q_mse_loss + scale*(bond_energy + angle_energy + torsion_energy + NB_energy)
        ...
        for param in network.parameters():
            if not torch.all(torch.isfinite(param.grad)):
                raise ValueError(f"gradients are NaNs/inf!!!")

        norm = torch.nn.utils.clip_grad_norm_(network.parameters(), 0.01, error_if_nonfinite=True) #0.01

        if verbose:
            wandb.log({"grad_norm": norm.item()})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no_phys", action="store_true", default=False)
